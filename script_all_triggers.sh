#!/usr/bin/env bash
#=============================================================================
# Script: script_all_triggers.sh
# Purpose: Export all trigger DDL (per table) into TABLES/TRIGGERS/ folder
# Usage:   bash script_all_triggers.sh -s SERVER -d DATABASE -u USER [-p PASSWORD | -k SECRET_ID] [-o SQL_DIR]
#=============================================================================
set -euo pipefail
...
if [[ -z "$SERVER" || -z "$DATABASE" || -z "$USERNAME" ]]; then
  echo "Usage: bash script_all_triggers.sh -s SERVER -d DATABASE -u USER [-p PASSWORD | -k SECRET_ID] [-o SQL_DIR]" >&2
  exit 1
fi
...
mkdir -p TABLES/TRIGGERS
rm -f TABLES/TRIGGERS/*.sql

echo "$tables" | while read -r table; do
  table=$(echo "$table" | xargs)
  [ -z "$table" ] && continue
  echo "Exporting triggers for table: $table"
  "$SQLCMD" -S "$SERVER" -d "$DATABASE" -U "$USERNAME" -P "$PASSWORD" -C \
    -i Script_Create_Trigger.sql -h -1 -W -m 1 \
    -v TableName="$table" -o "TABLES/TRIGGERS/${table}.sql"
  echo "  -> TABLES/TRIGGERS/${table}.sql created"
done

echo "All trigger exports completed!"
