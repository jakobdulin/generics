#!/usr/bin/env bash
#=============================================================================
# Script: deploy_triggers.sh
# Purpose: Deploy all trigger scripts from TABLES/TRIGGERS/ folder
# Usage:   bash deploy_triggers.sh -s SERVER -d DATABASE -u USER [-p PASSWORD | -k SECRET_ID] [-o SQL_DIR]
#=============================================================================
set -euo pipefail
...
if [[ -z "$SERVER" || -z "$DATABASE" || -z "$USERNAME" ]]; then
  echo "Usage: bash deploy_triggers.sh -s SERVER -d DATABASE -u USER [-p PASSWORD | -k SECRET_ID] [-o SQL_DIR]" >&2
  exit 1
fi
...
echo "Deploying triggers to $DATABASE..."

files=(TABLES/TRIGGERS/*.sql)
echo "Found ${#files[@]} trigger files"
...
echo "All triggers deployed successfully!"
